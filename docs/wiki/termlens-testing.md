...
| `card-status.test.ts` | `cardStatus()` / `cardHeading()`（`public/card-status.js`） | **後方互換** — `status` が無い旧カードを `confidence` から導出すること（`low` → `probable`、それ以外 `confirmed`）、`status` を `confidence` より優先すること（降格したカードが復元で戻らない）、どちらも無ければ `confirmed` に倒すこと。**`unresolved` の見出し**が `surfaceForms[0]` → `correctedFrom` → `term` の順に落ちること、`confirmed` / `probable` は `term` のままであること。#38 で **`mergeCardUpdate()` が `cardId` を書き換えない**ことを追加（更新のペイロードに `cardId` が紛れても素通しにしない。動くと2度目以降の更新が迷子になり、例外は出ないまま「確認中」が回り続ける）。据え置き（`unresolved`）の経路でも保たれること。#40 で **`rename` を伴う `card_update` だけが `unresolved` から戻せる**ことを追加 — 素の `card_update` では従来どおり据え置くこと（**#24 の回帰テスト**。昇格の穴を開けるついでにガードごと外しても、画面には「もっともらしいカード」が出るだけで例外は起きない）、`rename` があれば term / reading / 解説 / リンクが差し替わり `cardId` は動かないこと、**`shouldApplyResend()` は #40 でも不変**であること（速報の再送に `rename` は無いので #24 のガードがそのまま正しい） |
| `merge-duplicate.test.ts` | `mergeDuplicateCards()`（`public/card-status.js`） | **改名で同じ用語になった2枚の統合ルール**（#40）。Issue 本文が「実装前に決め、テストで固定する」と名指ししていた箇所。残す `cardId`（呼び出し側が渡す「登場順が早いほう」）が動かないこと、`surfaceForms` が 残す側 → 消す側 の順で連結され重複除去されること（消える側の表記を落とすと、その表記でハイライトされていた過去の行がどのカードも指さなくなる）、`links` が残す側優先で `MAX_LINKS`(3) まで補われること（**空配列で上書きして情報を失わない**）、残す側に無いフィールドだけが消す側から埋まること、引数を書き換えないこと |
| `terms-markdown.test.ts` | `buildTermsMarkdown()`（`public/terms-markdown.js`） | AC「Markdown エクスポートにも状態が残る」（#24）。`probable` の `※要確認`、`unresolved` の見出しが元の表記になり注記が付くこと、読みを出さないこと、Markdown 記法のエスケープ、http/https 以外のリンクを落とすこと。#38 で **`cardId` が Markdown に現れない**ことを追加（内部の識別子で読む人には意味が無く、セッションごとの通番なので載せると別の会議の Markdown と衝突する。出しても例外は出ないので固定しないと気づけない）。#40 で **改名後の内容が Markdown に出る**ことを追加（画面には直ったカードが出ているのにエクスポートだけ「特定できませんでした」のまま、という割れ方を防ぐ）と、**裏付けが取れなかったカードは unresolved の表記のまま出る**こと |
...
| `rematch.test.ts` | `toKana()` / `similarity()` / `isRelated()` / `mergeCandidates()` / `isResolved()` / `pickCandidate()`（`src/extract/rematch.ts`） | 再評価の純関数（#40）。仮名種別・全角半角・大文字小文字を潰すこと、類似度が**長いほうの長さ**で割られること（短いほうで割ると部分文字列が常に 1.0 になり、短い語が何にでも当たる）、**閾値が実際に効いている**こと（0.7 で通り 0.8 で落ちる境界を直接置く）、**`minSimilarity <= 0` が素通し**であること（`REMATCH_MIN_SIMILARITY=0` で分布を測るという手順の前提。ここが落ちると閾値を決める計測そのものができなくなる）、語に割ってから突き合わせること、1文字の語を類似度で通さないこと（1文字どうしは 1.0 か 0.0 にしかならず閾値が意味を失う）、`mergeCandidates()` が保存済み候補を先頭に保ち `MAX_CANDIDATES` で切ること、**`isResolved()` が候補外を弾く**こと（#23 の安全思想を再評価経路でも維持する） |
| `scheduler-rematch.test.ts` | `ExtractionScheduler` の再評価経路（`src/extract/scheduler.ts`） | **後続の確定用語で `unresolved` を昇格させ、同じ `cardId` で改名する**こと（#40 の本題）、改名に古い `surfaceForms` と `correctedFrom` がそのまま載ること、昇格した用語が `shownTerms` に載ること（載せ直さないと後続チャンクで同じ用語のカードがもう1枚出る）、昇格後は待ち行列から外れること。裏付けが取れない側は **棄却 / 候補外 / 例外のいずれでも `card_update` を送らず `unresolved` を維持**すること。コスト上限は5つとも個別に固定する — 無関係な確定カードでは検証を投げないこと、**同じチャンクの確定カードでは回さない**こと（抽出段が両方見たうえで unresolved にしているので新しい材料が無い）、`MAX_REMATCH_ATTEMPTS` で止まること、cooldown 中は回さないこと、1 run あたりの上限、`MAX_PENDING_UNRESOLVED` を超えたら**古いほうから捨てる**こと、`verifyDisabled` で清書ごと止まること。#25 のプライバシー原則も本番の経路で見る（用語集は関連語だけ・参加者名は渡らない・会話全文を渡さない）。**時計を差し替えてある** — cooldown と試行回数は実時間に依存するので、待たずに跨げないと確かめられない。清書と同じ関数を通るので、再評価の呼び出しは入力の「音声認識が崩した元の表記」で見分ける |
...
| `config-env.test.ts` | env ノブの検証（`src/config.ts`） | `MAX_WEB_SEARCHES` が整数でなければ**起動時に落ちる**こと（`2.5` が通ると `max_tool_calls: 4.5` になって API 側 400 → そのセッションの検証が丸ごと止まる）、空文字は未設定と同じ扱いになること、`0` が「上限なし」として通ること。#40 で再評価のノブを追加 — `REMATCH_MIN_SIMILARITY` が 0..1 の実数で**範囲外は起動時に落ちる**こと（`5` はどの語も通さない設定として黙って効き、**再評価が一度も発火しないまま静かに死ぬ**）、**`0` が「絞り込みなし」として通る**こと（これが閾値を決めるための計測手順の前提）、`MAX_REMATCH_ATTEMPTS` / `REMATCH_COOLDOWN_MS` が整数ノブであること。`config.ts` は import した瞬間に検証するので別プロセスで確かめる |
...
| `eval-metrics.test.ts` | `src/eval/metrics.ts` | 指標の計算そのもの（LLM 抜き）。**`probable` と `unresolved` を別々の列に数える**こと（#24。合算すると過剰 unresolved に気づけない）。#40 で再評価の2指標を追加 — `expectRematch` の `from` と `to` の**両方**が一致して初めて正解に数えること（`to` だけ見ると別の未解決語がたまたま期待した用語に着地した場合も加点され、誤補正率が過小に出る）、**再評価が一度も走らなかったランを「昇格率 100%」にしない**こと（分母 0 の向きが Recall 側と逆） |
...
#40 で「**1度 unresolved にした語を後続の文脈で直せるか**」のケースを1件足した。

- `rematch-garbled-then-clear` — 崩れた仮名表記と、同じ概念の明瞭な表記が同じ
  `transcript` に出る。`expectRematch: [{ from, to }]` に「聞き取られた表記 → 確定して
  ほしい用語」を置く。抽出段が1回で正しく補正してしまえば再評価は発火せず、
  指標は 0 のまま（**それは失敗ではない**）

**既存の `unresolved-*` 2件は `expectRematch` を空のままにしてある。** あの2件は
「断定しないのが正解」なので、**昇格したら必ず誤補正に数えられる**のが正しい。
`expectRematch` の既定が `[]` なのでそのまま期待どおりに働く。
...
| 再評価の昇格率 | 昇格した数 ÷ 再評価を試みた数（#40） | （報告のみ） |
| **再評価による誤補正率** | 期待と違う用語へ昇格した数 ÷ 昇格した数（#40） | （報告のみ） |
...
- **再評価の2指標は必ずセットで読む**（#40）。`unresolved` 率だけを下げることを成功条件に
  しない — 誤って何でも確定すれば `unresolved` 率は下がる。昇格率だけが上がって
  誤補正率も上がっていたら、`REMATCH_MIN_SIMILARITY` を締める
- **分母 0 のときの向きが指標ごとに違う。** Recall / 補正 / Precision は「期待が無いことは
  失敗ではない」ので 1（減点しない）。誤補正率と再評価の2指標は 0（良い側）。
  再評価の昇格率を 1 にしないのは、**機能が一度も発火していないランが「昇格率 100%」に
  見える**のを防ぐため
...
- **再評価の発火頻度**（#40）— 評価ハーネスは1ケース＝1チャンクなので、
  「後続のチャンクで確定したカードを手がかりにする」という**時間差そのものが作れない**。
  ハーネスが測れるのは**判断の質**（正しく昇格したか / 誤って昇格したか）だけで、
  実際の会議で発火するかは実機で見るしかない
- **統合が起きたときの表示**（#40）— 統合ルール（`mergeDuplicateCards()`）と配線
  （`app-wiring.test.ts`）は固定してあるが、DOM が実際に1枚に畳まれてスクロール位置が
  飛ばないかはブラウザでしか確かめられない
