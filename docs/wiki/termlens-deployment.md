...
`fly machines list` で `suspended` / `CHECKS 0/1` と出るのは `auto_stop_machines` による
正常なアイドル休止。接続中の WebSocket があれば休止しない。

### アイドル復帰は "suspend" を使う

初回アクセスの待ち時間を実測して比較した（2026-08-19、nrt、shared-cpu-1x 512MB）。

| 状態 | 初バイトまで |
|---|---|
| 起動中 | 0.03 秒 |
| `stop` からの起動 | **7.2 秒** |
| `suspend` からの復帰 | **0.39 秒** |

**停止中の課金は `stop` と `suspend` で同じ**（rootfs の $0.15/GB/月 のみ。CPU と RAM は
どちらの状態でも課金されない）。つまり追加費用なしで初回表示が約 18 倍速くなるため、
`suspend` を選ばない理由が無い。

注意点:

- **デプロイ直後の初回アクセスは従来どおり起動待ち**になる。新しいマシンには保存された
  メモリが無いため
- suspend が常に成功するとは限らず、その場合は従来どおりの停止・起動になる
- 復帰後の Node プロセスは休止前の状態を引き継ぐ。HTTP の keep-alive 接続が切れている
  可能性があり、復帰直後の 1 回目の API 呼び出しが失敗しうる。抽出は一時エラーとして
  再試行するので実害は出ない想定だが、頭に入れておくこと
- 常時 0.03 秒にしたい場合は `min_machines_running = 1` にできるが、こちらは CPU と RAM が
  常時課金される
