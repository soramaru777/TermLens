...
##### 閾値（`public/utterances.js`。#48 の 3 つは 1 サンプル、#59 の 2 つは 2 サンプル目、#61 の 5 つは 3 サンプル目由来の暫定値）
...
| `MINOR_ISLAND_MAX_WORDS` | 20 | 1 つの島として吸収してよい最大 word 数。**③b の「長い run」の下限でもある** |
| `MIN_TOTAL_WORDS_FOR_ISLANDS` | 200 | これ未満では主要 speaker の順位を信用しない |
| `LONG_MINOR_NEUTRALIZE_MAX_RATIO` | 0.03 | ③b が長い run を中立化してよい minor 比率の上限（未満）。`MINOR_ISLAND_MAX_RATIO` 以下であることをテストで固定（#61） |
| `LONG_MINOR_MAX_RUNS` | 1 | 同じ speaker の長い run がこれを超えたら③b は中立化しない（#61） |
| `LONG_MINOR_MAX_WORDS` | 60 | ③b の 1 run の word 上限。**再帰属と中立化の両方**に掛ける（#61） |
| `LONG_MINOR_MIN_MERGE_SEGMENTS` | 2 | ③b の再帰属の根拠 E1 に要る safe merge の segment 数（#61） |
| `LONG_MINOR_TRANSITION_BIAS` | 0.75 | ③b の根拠 E3 で「遷移が片方の major に偏っている」とみなす割合。遷移総数 2 以上のとき（#61） |
...
`absolute` と判定されたが、その minor cluster が **`B → X(長) → A` の 1 本の長い run** で、②は
`mismatch`、③は `run が長い` で見送り、表示上の通常話者数が 3 のまま残った。②③が長い run を
触らないのは「誤割り当てされた本物の発話でありうる」ためで、その安全弁は据え置いたまま、
③の後ろに **③b `planLongMinorRuns()`** を足した。

##### 位置と入力

- 入力は **③の戻り `unresolvedPlan.skippedRuns` のうち `reason === "tooLong"`** だけ。②が
  `A → X(長) → A` に付けた `tooLong` も、③が `B → X(長) → A` を `mismatch` から付け替えた `tooLong` も
  ③は素通しで返すので両方ここへ来る
- **run は切り直さない**（③と同じ規律）。`boundary` / `unknown` / `edge` の run は②の時点で `tooLong` に
  なれないので、③b には来ない。「再接続境界を跨がない」はこれで担保される
- **ゲートは②と同一**。②が `disabledBy` なら③b も同じ理由で無効（③と同じ）。計画が無ければ `noPlan`
- 走査する行は⓪①通過後の配列（前後の行の `seq` / `text`）、遷移の統計は raw の
  `collectSpeakerStats().transitions`。②③の適用は行数も並びも変えないので、③b の添字はそのまま
  合成後の配列に当たる
- 適用は②③と同じ 2 つの操作の再利用（`applyMerges()` に `{ merges: attributed }` を、`applyNeutralize()` に
  ③b の計画を渡す）。③b 専用の適用関数は持たない。**復元データ由来の `unresolved` を剥がすのは
...
| kind | 何を見るか | 出どころ |
|---|---|---|
| `merge`（E1） | 同じ minor X の別 run が②で safe merge されている。帰属先が **1 つの major だけ**で `segments >= LONG_MINOR_MIN_MERGE_SEGMENTS` | `plan.merges` の `from === X` |
| `seq`（E2） | run の先頭行が直前の major 行と同じ `seq`、または末尾行が直後の major 行と同じ `seq`（前後それぞれ独立。隣が major でなければ根拠にしない） | ⓪①通過後の行の `seq` |
| `continuity`（E4） | 境目の文字種の連続性が `medium` 以上、かつ手前側の行が句読点で閉じていない（前後それぞれ独立） | 既存の `continuity()` / `isPunctuated()`（#57 と同じ規則） |
| `transition`（E3） | X を含む raw 遷移のうち片方の major が `LONG_MINOR_TRANSITION_BIAS` 以上を占める。分母は X を含む全遷移。総数 2 未満なら根拠にしない。**再帰属の裏付けには数えない**（下の注記） | `stats.transitions` |

「境界の時間差」は行に無いので使わない（サーバーから分割点の gap ms を送るなら別 Issue で `kind` を足す）。
同じ種別・同じ major は 1 つに畳む（`A → X → A` で前後の両方が当たる形）。

##### 判定（1 run に結論は 1 つ）

1. E1 が **2 つ以上の major に割れていれば** `kept: conflictingEvidence`（再帰属も中立化もしない）
2. **再帰属**（すべて満たす）: E1 が一意 ∧ **E2 / E4 のどちらか**が同じ major を指す ∧ 他の major を指す
   根拠（E3 を含む）が 1 つも無い ∧ `words <= LONG_MINOR_MAX_WORDS`。
   **E3 を裏付けに数えないのは E1 と独立でないため。** E3 は raw の遷移を数えるので、E1 の根拠になった
   safe merge 済みの島 `A → X → A` がそのまま `A` 側に 2 遷移ずつ積まれる。島が 3 本あれば run の両隣が
   別の major でも 6/8 = 0.75 に届き、E1 + E3 だけで「複数の独立した根拠」に見えてしまう。裏付けは
   run そのものの境目を見る E2 / E4 に限り、E3 は反対根拠と診断の「根拠:」にだけ使う（テストで固定）
3. **中立化**（再帰属できず、すべて満たす）: `minorJudgements` の種別が `absolute` ∧
   `ratio < LONG_MINOR_NEUTRALIZE_MAX_RATIO` ∧ その speaker の長い run（③b の候補）が
   `LONG_MINOR_MAX_RUNS` 本以下 ∧ `words <= LONG_MINOR_MAX_WORDS`
4. **維持**: 上のどれかで落ちた run。理由の優先順位は
   `conflictingEvidence → notAbsolute → ratioTooHigh → manyRuns → runTooLong`

観測サンプルは E1 が無いので再帰属に進めず、`absolute` / 2.6% / 1 本 / 26 word で**中立化**に落ちる。
再帰属は E1 が実データで 0 件なので、**実データでは眠ったままになる**設計（誤帰属は相手の発言が
major の名前で本文に残るので中立化より害が大きい。入口を「合成 fixture でしか通らない」状態にしてある）。

##### 調査コメントからの調整点

1. **中立化を止める「反対根拠」は E1 の割れ（`conflictingEvidence`）だけ。** E2〜E4 が major を指すのは
   「X はその major の発話が誤割り当てされたもの」という向きの根拠で、**第三者らしさの根拠ではない**。
   `B → X → A` の X が両隣と同じ final にあるのは「A と B の境目に挟まった断片」の形そのもので、
   これで中立化を止めると観測サンプルの形が条件次第で `kept` に落ちる。第三者らしさは比率・run の
   本数・run の長さで測り、E2〜E4 は再帰属の可否だけに効かせる
2. **`LONG_MINOR_MAX_WORDS` は再帰属にも掛ける。** Issue の再帰属条件には長さの上限が無いが、
   誤帰属は中立化より害が大きいので、中立化より緩い条件にしない
3. `LONG_MINOR_NEUTRALIZE_MAX_RATIO`（3%）は `MINOR_ISLAND_MAX_RATIO` と同じ値なので、現行値では
   `absolute` 種別のゲートと同じ意味になり **`ratioTooHigh` は単独で効かない**。後から 2% に締めたくなった
   ときのために種別ゲートと比率定数は分けてある（判定順はテストで固定）

##### 既存テストへの影響

③の「`tooLong` の run は中立化しない」を `groupUtterances()` 経由で断言していたテスト 2 本は、
③b が同じ run を引き取って中立化するため（絶対 minor・run 1 本・上限以下）、**③の計画では中立化
していないこと**と**③b が引き取っていること**を断言する形に書き換えた。③の判定そのものは変えていない。

##### 診断への出方

`### 表示補正（minor island）` の中で③の行の直後に出る（画面パネルと Markdown は同じ配列から）。
会話本文は 1 文字も出ない。

```
- 長い minor run: 1 run
- 長い minor run の再帰属: 0 run / 0 word
- 長い minor run の再帰属の根拠: safe merge 0 / 同一 final 0 / 文字種 0 / 遷移の偏り 0
- 長い minor run の中立化: 1 run / 26 word
- 長い minor run の中立化 2 → 話者不明: 1 run / 26 word
- 長い minor run の維持: 根拠が割れている 0 / 絶対判定でない 0 / 比率が高い 0 / run が複数 0 / run が長い 0
- 長い minor run の閾値: 比率 3.0% / run 1 本 / 60 word / safe merge 2 seg / 遷移偏り 75.0%
- speaker 2 (長い run): 絶対 2.6% / 1 本 / 中立化 / 根拠: 文字種→1, 文字種→0
```

再帰属があれば `長い minor run の再帰属 2 → 0: 1 run / 30 word（根拠: safe merge, 同一 final）` の行が
from→to ごとに出る。speaker 別の行の「根拠」は結論にかかわらず集めた全部（維持でも「何が見えていたか」
が読める）。表示名は `LONG_MINOR_KEEP_LABELS` / `LONG_MINOR_EVIDENCE_LABELS`（`diagnostics.js`。
キーは `utterances.js` で、順序一致をテストで固定。#59 の `MINOR_KIND_LABELS` と同じ流儀）。
無効時は `長い minor run: 無効（…）` の 1 行、計画を渡されていなければ行ごと出さない（②③と同じ）。

**読み方**: `絶対判定でない` が多ければ相対 minor を対象に含めるか、`run が複数` / `run が長い` が
多ければ `LONG_MINOR_MAX_RUNS` / `LONG_MINOR_MAX_WORDS` を、`根拠が割れている` は本物の第三者の疑い。
再帰属が実機で 1 件でも出たら、`根拠` の内訳と本文を突き合わせて誤帰属でないかを人が確かめる。

#### 同じ final の中で語の途中に入った speaker 境界を平滑化する（Issue #55）

**表示側の補正は⓪が先頭に加わって 4 段になった**（グループ化まで含めて 5 段）。順序は
...
##### 判定の単位は「同じ final の中で連続する短い行の run（chain）」で、2 段（Issue #57）

> 2026-09-06 まで（#55）: 判定の単位は 1 行。3 文字以下の断片を、同じ `seq` の「断片より長い隣」へ
> 寄せていた。句読点で閉じた断片は絶対に寄せず、隣も断片なら `shortNeighbor` で見送り、4 文字以上は
> 判定の対象外だった。実機の 2 名ポッドキャストで「語途中で切れて後半に句読点が付く」「4〜5 文字の
> 明らかな続き」「短断片が交互に続く chain」の 3 パターンが取りこぼされていたため、#57 で拡張した。

行を長さで 3 段（tier）に分ける: **tier 0** = 6 文字以上（`BOUNDARY_EXTENDED_CHAR_LIMIT` 超）/ 空 /
再接続の印、**tier 1** = 通常断片（`BOUNDARY_FRAGMENT_CHAR_LIMIT` = 3 文字以下）、**tier 2** = 拡張断片
（4〜5 文字）。同じ `seq` の中で tier 1・2 の行が連続する極大 run を **chain** とし、chain ごとに判定する
（`public/utterances.js` の `smoothSpeakerBoundaries()`）。

**(a) 長い anchor による chain 判定**（`resolveByAnchors()`）。chain の両隣の同じ `seq` の行は tier 0 なので、
あれば必ず anchor の長さ。

1. anchor が片側だけ、または両側が同じ speaker → その speaker が寄せ先。両側とも無ければ (b) へ
2. 両側の anchor が別 speaker でも、chain が**先頭から左の speaker・途中から右の speaker と単調に
   並んでいれば「既に割れている」**（#55 の「同じ話者の本体に接している断片は対象外」の一般化）。
   何もせず見送りにも数えない。それ以外（`A長 | X短 | B長`、交差）は (b) へ落ちる（`ambiguous` に
   数えるのは (b) で境界に立つ tier 1 の行だけ。tier 2 の X は数えない）
3. 寄せる行（寄せ先と speaker が異なる行）の総文字数が `BOUNDARY_CHAIN_MAX_CHARS`（8）を超える →
   候補になった tier 2 の行を `chainTooLong` に数えて (b) へ落とす（tier 1 の行は (b) で数える）。
   「短い語の応酬」を丸ごと吸う事故を止める総量ゲート
4. 内容ゲート（`contentGate()`。判定順は 語彙 → 句読点 → 文字種）
   - 末尾の句読点を**剥がして** `BACKCHANNEL_WORDS` に当たる → `backchannel`（`はい。` を
     「句読点で閉じている」ではなく「相槌語彙」に数える。語彙は 5 文字まで載せられる）
   - 句読点で閉じた行: **直前の行との文字種の連続性**が「中」以上で、直前の行が句読点で閉じていなければ
     前の語の続きとして通る。満たさなければ `punctuated`（意味を「語尾の証拠が無い」に変えた）。
     次の行との境目は見ない — 句読点で閉じた行の後ろに続く語は無い
   - tier 2 の行: 前後どちらかの境目の連続性が「中」以上なら通る。そうでなければ `weakContinuity`
   - tier 1 で句読点の無い行: 文字種は見ない（#55 の挙動を変えない）
   - **全部か無しか**。1 行でも落ちたら chain 全体を寄せない（部分的に寄せると chain の途中で
     speaker が入れ替わり、観測より複雑な形を作る）
5. 通れば寄せる行すべてを寄せ先の speaker にし、`applied` に `kind` を付ける

**(b) #55 互換の run 判定**（`resolveByRuns()` / `resolveRun()`）。(a) で決まらなかった chain に対し、
**4 文字以上（tier 0 と tier 2）を本体、tier 1 だけを断片**として #55 の規則（`classifyNeighbor()`）を
tier 1 の run に当てる。本体に接している側からはその speaker の行を順に「本体の一部」として外し、
残った run の両隣から寄せ先を取る。1 行の run に対しては #55 と同じ結果で、2 行以上の run
（`A:テ | A:キ | B:ストです`）は #55 が `キ` だけを寄せていた形をまとめて寄せる（パターン 3 の修正）。
総量と内容ゲートは (a) と同じ。

**(b) を残すのは、4〜5 文字の行を一律に断片にすると #55 で本体だった行が断片になるため。**
`今日は | 晴れですね` のような「短い断片 + 4〜5 文字の本体」は (b) で従来どおり寄る。テストの fixture
（⓪の `BODY` = 4 文字、①の `SHORT` = 4 / `LONG` = 5 文字）がすべて 5 文字以下なのは偶然ではなく、
この経路が #55 と #36 の既存テストを構造的に守っている。(a) と (b) が食い違う形
（`A長 | B:4〜5文字 | A:短`）は、(a) が「6 文字以上の本体が同じ final にある」という #55 より強い証拠を
要求しているので **(a) を優先**する。(a) が内容ゲートで落ちれば (b) に落ちる。

文字種の連続性（`continuity()`。辞書は使わない。日本語前提で、英数は 1 クラス）:

| 前の末尾 → 後の先頭 | 強さ |
|---|---|
| カタカナ → カタカナ（長音「ー」を含む）/ 漢字 → 漢字（「々」を含む）/ 英数 → 英数（全角を含む） | 強 |
| 漢字 → ひらがな（送り仮名・助詞）/ ひらがな → ひらがな | 中 |
| ひらがな → 漢字（助詞 → 名詞の典型的な語境界）など上記以外 | 弱 |
| どちらかが句読点・空白・空 | 無 |

**理由の判定順は「構造（seq・隣）→ 総量 → 内容（語彙・句読点・文字種）」。** 逆にすると別 final の「はい」が
`backchannel` に数えられ、語彙リストを調整するための件数が構造上どのみち寄らないケースで
水増しされる。内容ゲートの件数は「それが無ければ寄っていた」数だけになっている。

**見送りの計上は 1 行 1 理由。** (a) で数えた行を (b) で数え直さない（`decided`）。**tier 2 の行を
数えるのは (a) の候補になったときだけ**（`weakContinuity` / `punctuated` / `backchannel` / `chainTooLong`）。
(b) では本体側なので数えない — 数えると「4〜5 文字の行の大半が見送り」になって内訳が読めなくなる。
**tier 0 の行は数えない**（#55 と同じ。内訳に出るのは「短いのに寄せなかった行」だけ）。run の中で
全行が同じ speaker で両隣も境界でなければ判定の対象外で何も数えない。

`A長 | A:あ | A:い | B長` は #55 が 1 行の窓で `い` を B へ寄せていたが、#57 では両方とも A の本体の一部と
見て何もしない（raw が A と言っている連続を割る根拠は無い）。

**①への影響**: ⓪(a) が 4〜5 文字の行を寄せられるようになったので、同じ final の中で 6 文字以上の同じ
speaker に挟まれた 4 文字の島は ⓪ が先に寄せ（`kind: extended`）、①（`JITTER_CHAR_LIMIT` = 4）の出番が
減る。結果の speaker は同じで矛盾しない。①は計画を出さないので診断上の二重計上もない。逆に、
①は語彙を見ずに「同じ final で同じ話者に挟まれた 4 文字以下」を寄せるので、`A: 進めます。 | B: はい。 |
A: 次です。` の `はい。` は⓪では `backchannel` で見送られても①で A に入る（#36 以来の挙動。⓪のテストは
⓪単体の出力で断言している）。
...
```
- 表示補正（speaker boundary）: 6 seg / 15 文字（通常 3 / 4〜5文字 1 / 句読点付き 1 / chain 1）
- 境界補正の見送り: 曖昧 2 / chain 未解決 3 / chain が長い 0 / 連続性が弱い 1 / 句読点で閉じている 1 / 相槌語彙 4 / 隣が別 final 12 / 再接続境界 0 / 話者不明 0 / seq なし 0
```

> 2026-09-06 まで: 1 行目は `3 seg / 7 文字` だけ（種別の内訳なし）。見送りの内訳に `隣も断片`
> （`shortNeighbor`）があり、`chain 未解決` / `chain が長い` / `連続性が弱い` は無かった。

単位は常に文字数（行に `w` があっても断片の一部を word 数では表せない）。会話本文も speaker
番号の明細も出さない。1 行目の括弧は `applied[].kind` の内訳（#57）で、**どの規則が寄せたか**を見る
ためのもの。`kind` は 1 行 1 種類で、リスクの高い規則から優先して付ける: 句読点で閉じていた行 →
`句読点付き`、4〜5 文字の行 → `4〜5文字`、2 行以上をまとめて寄せた run の行 → `chain`、それ以外
（#55 相当）→ `通常`（`utterances.js` の `appliedKind()`、表示名は `diagnostics.js` の `BOUNDARY_KIND_LABELS`）。

**見送りの内訳が閾値と語彙を決める唯一の材料** — `連続性が弱い` が多ければ文字種の表、`chain が長い` が
多ければ総量の上限、`曖昧` が多ければ両隣が別話者の長い行に挟まれた形、`chain 未解決` が多ければ
anchor の無い final が多い、`相槌語彙` が多ければ語彙リストが効いている、と読む。
...
`skipBreakdown()` 1 か所（種別の内訳も同じ関数）。**既存のキーは改名しない** — 過去セッションの診断
Markdown と件数を比べられなくなる（#57 で Issue が挙げていた `punctuationStandalone` などへの改名は
見送った）。
...
| 定数 | 値 | 場所 |
|---|---|---|
| `BOUNDARY_FRAGMENT_CHAR_LIMIT` | 3 | `public/utterances.js`。通常断片（tier 1） |
| `BOUNDARY_EXTENDED_CHAR_LIMIT` | 5 | `public/utterances.js`。拡張断片（tier 2）の上限。これより長い行が (a) の anchor（#57） |
| `BOUNDARY_CHAIN_MAX_CHARS` | 8 | `public/utterances.js`。1 つの chain / run で寄せる行の総文字数の上限（#57） |
| `BOUNDARY_CONTINUITY_MIN` | `medium` | `public/utterances.js`。文字種の連続性を「語の途中」と認める最低の強さ（#57）。`4〜5文字` の適用が不自然に多ければ、語彙より先に `strong` へ上げることを検討する |
| `BOUNDARY_PUNCTUATION` | `。、？！?!` | `public/utterances.js`。#57 から絶対条件ではなく「直前との連続性を要求する印」 |
| `BACKCHANNEL_WORDS` | はい / ええ / うん / そう / へえ / あー / えー / ん / はー / ほう / ふむ + なるほど / そうです / そうですね / はいはい / 了解です / ですよね / わかった / ありがとう / お疲れ様 + ひらがなだけの頻出相槌（うんうん / そうそう / たしかに / ちょっと / えっと / えーと / あのー。「ひらがな → ひらがな」が「中」なので語彙で止める） | `public/utterances.js`（5 文字以下の語だけ。長い語は長さのゲートで先に落ちる。句読点を剥がして完全一致。4〜5 文字の語は #57 の暫定リスト。⓪の調整つまみは 6 つともこのファイルの同じ節にある） |
| 文字種の連続性の表 | 上記 | `public/utterances.js` の `continuity()`（`tests/utterances.test.ts` が表をそのままテストにしている） |

**残るリスク**: 重なって話して endpointing の無音が入らず、相手の相槌が同じ final に混ざった
ケースは、語彙・句読点で弾けなければ隣の話者へ寄る。**テキストは消えず帰属だけが変わる**。
#57 で 4〜5 文字と句読点付きの行まで対象になったので誤統合の面積は #55 より広い（語彙に無く連続性が
「中」以上の 4〜5 文字の返事は寄る）。頻度は診断の `4〜5文字` / `句読点付き` / `chain` の件数で追う。
文字種の表は日本語前提で、英語の語境界は判定できない。chain の「全部か無しか」は取りこぼしを生む
（1 行だけ語彙に当たる chain は丸ごと見送る）が、安全側の倒し方として意図的にそうしている。
...
> 2026-09-05 追加: 実機で「発話の冒頭が数文字欠けて見える」ケースが複数観測された。
> **本当に文字が落ちているのか、話者分割の位置の問題なのかを推測せずに切り分けるため**、
> 段階ごとの文字数を測る計測だけを入れた。**この Issue では症状を直していない。**

```
Deepgram Results
  │  channel.alternatives[0].transcript                 ← ① raw
  ▼
buildTranscriptEvents()        [deepgram.ts]
  └─ buildFinalEvents()        [split.ts]  → { events, diag }  ← ② 分割後
  ▼
SplitIntegrity.add(diag)       [stt/integrity.ts]  セッション累計
  ▼  ServerMessage: text_integrity（final ごと）
──────────────────────────── WS ────────────────────────────
  ▼
app.js: finalLines へ push                                     ← ③ 受信
  ▼
groupUtterances()（⓪①jitter → ②island → ③中立化 → ③b → ④結合）  ← ④ 表示
  ▼
diagnostics.js: 「STTテキスト完全性」節（①②③④を1つの表に）
```

### 文字数は「空白を除いた文字数」で測る

**これが設計の要。** `sliceFromTranscript()` は切り出しに `.trim()` を掛けるので、
**正常に動いていても素の `length` は減る**。素の文字数で段階を比べると「正常な空白の消失」と
「本物の欠落」が混ざって読めない。

空白を除いた文字数（`visibleChars`、正規表現は `/\s/g`）は**切り出しに成功しているかぎり**
保存される。**保存されていれば分割、減っていれば欠落**。この判定基準は診断 Markdown の節にも
書いてある（表だけ残すと、別の日に読んだ人が素の文字数の差を欠落と読み違える）。

#### フォールバックした final は例外

**空白を除いた文字数も保存されない。** フォールバックは `sliceFromTranscript()` が失敗した
とき、すなわち `transcript` と `words[]` が**食い違う文字列だった**ときにだけ起きる。
`join("")` で組み直した ② は words 側の文字列なので ① とは別物で、増減の両方向が起きる。
...
診断は `fallbacks > 0` のとき①→②の差を**別扱いの判定文**にする
（`public/diagnostics.js` の `INTEGRITY_FALLBACK_VERDICT`）。ここを普通の増減として
読ませると、切り出しも破棄も起きていないのに「話者分割で欠落している」あるいは
「同じ Results を二重計上している」と断定し、存在しないバグを追わせることになる。

定義は言語境界をまたぐので実装が2つある（`src/stt/split.ts` と `public/diagnostics.js`）。
`tests/integrity.test.ts` が全角空白・タブ・改行まで含めて両者の一致を突き合わせている。

### 計測は `buildFinalEvents()` の戻り値に足す

**並走する計測関数を作らない。** `split.ts` の冒頭にあるとおり同じ規則を2か所に書くと片方だけ
直したときに静かに食い違う。`measureFinalSplit()` のような関数を別に置くと切り出しの規則が
2実装になり、「計測器のほうが古い」状態を作れる。`buildFinalEvents()` は
**依存ゼロ・純関数のまま**で、戻り値が `{ events, diag }` になっただけ。カウンタは持たない
（セッション累計は `src/stt/integrity.ts` の `SplitIntegrity` が積む。`session.ts` に置くと
`config.ts` を連れ込むテストになるので、`split.ts` / `utterance.ts` と同じくファイルを分けた）。

`SplitDiag` が持つのは `rawChars` / `rawVisible` / `splitChars` / `splitVisible` /
`segments` / `events` / `fallback` / `headDropped` の8つ。

**`headDropped` を独立した boolean にしてある。** #52 の調査で「発話の冒頭が丸ごと消える」を
構造的に説明できた唯一の経路が「フォールバック後に先頭セグメントの全 word が空表記で、
`.filter(text.length > 0)` に落ちる」だったため。件数（`segments - events`）に埋めると
「何件落ちたか」は分かっても「頭が落ちたか」が読めない。

### `diag` を配るのはテキストのある final だけ

interim と空 transcript の Results には付けない。interim は同じ発話を何度も出すので積むと
文字数が数倍に膨らみ、空 transcript を数えると `finals` が「沈黙の回数」になる。

### 診断に出るのは数字だけ（hash も入れない）

Issue は「必要なら本文を復元できない hash」としているが、**入れていない**。短い発話は総当たりで
復元されうるので、まず入れずに済ませる。出るのは件数・文字数・差分・timing・boolean だけ。

`ServerMessage.text_integrity` は**キーを明示して並べる**（`session.ts`）。`{ ...snapshot }` に
するとオブジェクトリテラルへの excess property check が効かず、`IntegritySnapshot` に
フィールドが1つ増えただけで型エラーゼロのままクライアントへ出ていく（#46 のレビューで
`stt_info` について潰したのと同じ形）。クライアント側も `app.js` でキーを挙げて持つ。

送信は **final ごとで、スロットルは入れていない**。数値10個ほどのペイロードで、載せないと判断した
`words` 配列（#19 で約13倍）とは桁が違う。間引くと「いつの時点の累計か」を持つ状態が増えるだけ。

**累計は `localStorage` に保存しない。** 復元した値はセッションをまたいだ集計になり、同じ表に
並ぶ③④（今の画面の行から数える）と対応しなくなる。

### ③④はクライアントで数える

③は raw の `finalLines`、④は `groupUtterances()` の出力。**④は表示・エクスポートと同じ引数で
呼ぶこと** — 別の引数で呼ぶと、画面に出ていない補正結果の文字数を診断が報告する。
`tests/app-wiring.test.ts` が呼び出し文字列を固定している（#48 と同じ規律）。

③を `groupUtterances()` から数えると②→③ の差が常に 0 になり、④を `finalLines` から数えると
③→④ の差が常に 0 になる。**どちらも例外は出ず表は埋まったまま**なので、数字を読んだ人は
「一致している」と結論する — 測定器が壊れていても壊れたと分からない形になる。

#### ③④は最後の再接続以降だけを数える

①②を持つサーバー側の `SplitIntegrity` は **WS 1本ごと**に作り直されるのに、クライアントは
再接続しても `finalLines` を保持する。境界で切らないと③④だけが前のセッションぶんを含んだまま
比較され、**②→③ に実在しない増加**が出る。再接続はアプリが正規にサポートする経路
（指数バックオフで最大5回）なので、この計測が答えるべきまさにその場面で表が読めなくなる。

境界の印は #36 が入れた `{ type: "reconnect" }` をそのまま使う（`finalSeq` が同じ問題を
持たないのも、この印が番号の振り直しをそこで吸収しているから）。**範囲を切るのはグループに
してから** — #48 の統合は全体の語数比で決まるので、切った行から組み直すと画面に出ているのと
違うグループを数えることになる。

**同じ分岐で累計そのものも捨てる。** ③④を切るだけでは逆向きの誤診断が残る — サーバー側は
新しい WS で 0 から数え直すのに、クライアントが前のセッションの `text_integrity` を持ち続けると、
新しい final が1件届くまで「①②＝前セッション分 / ③④＝0」の表になり、判定文が
「クライアントが取りこぼしている」と言う（**何も落ちていない**）。無音が続けばこの状態は
いくらでも長引く。`textIntegrity = null` に戻せば、次の1件が届くまで節ごと出ない。

### `vad_events` / SpeechStarted は入れなかった（方針4の見送り）

Issue の方針4は「`SpeechStarted` を受けて『VAD が発話開始と判断した時刻』と『直後の final の
先頭 word の `start`』の差を測る」というもので、一度実装したうえで**外した**。理由は2つ。

**1. 差が欠落の証拠にならない。** 公式ドキュメントが「SpeechStarted の timestamp は次の
transcript の最初の語の開始時刻と必ずしも一致しない（転写と語のタイミングは独立に動く）」と
明記している。つまり gap が大きくても「冒頭を拾えていない」とは言えず、この計測が答えるべき
問いには届かない。

**2. 通らなかったときの壊れ方が重い。** `vad_events` が現行の設定（日本語 + `diarize=true`）で
通るかは実接続で確認できていない。ハンドシェイクで拒否されると `start()` が reject して
**STT が一度も開始しない** — #46 の `diarize` / `diarize_model` 同時指定が 400 になる件が
まさにこれで、「イベントが来ないだけ」では済まない。未検証のパラメータのために全セッションの
起動を賭けるのは、補助的な観測と釣り合わない。

必要になったら、パラメータを外して再接続するフォールバックとセットで入れること。

### 分類 C（UtteranceEnd の順序）はこの Issue に含めていない

Issue の方針3（`UtteranceEnd.last_word_end` と final の順序）は実装していない。
`session.ts` の配線は2本に分かれており、**画面の文字起こしは `transcript` イベントの `text` を
そのまま積んだもの**で `UtteranceBuilder` を通らない。`UtteranceEnd` が対応する final より早く
届いて発話が一足早く閉じても、**用語抽出に渡る文脈の切れ目が変わるだけ**で表示される文字は
1文字も変わらない。したがって表示上の冒頭欠落の切り分けには効かない。

「推測で直さない」のと同じく、**推測で測らない**。順序の観測が要るのは抽出品質の話で、別 Issue。

### この計測で何が分かるか

| 実機の観測 | 結論 |
|---|---|
| 全段で空白除く文字数が一致 | 文字は失われていない。症状は `splitBySpeaker()` の分割位置（word 単位で助詞1語だけ話者が変わる）による見え。分割粒度は独立した設計判断（上の「最小語数の閾値は入れていない」）として別 Issue へ |
| ①→②で減 | 話者分割で落ちている。`fallbacks` / `headDrops` / `droppedEvents` がそのまま原因を指す |
| ①→②で増減（`fallbacks > 0`） | フォールバック由来。②は①と別の文字列なので、この差は欠落ではない |
| ②→③で減 | サーバーが送った final をクライアントが取りこぼしている |
| ③→④で減 | 表示側の回帰。#36 / #48 / #50 はラベルしか変えない設計なので、減っていれば不具合 |
| ①の時点で体感と合わない | Deepgram 以前（収音条件・`endpointing`・モデル設定）へ絞れる |

**未検証**: 実機データはまだ取っていない（[[termlens-open-issues]]）。
